#!/usr/bin/env bash
# ============================================================
# Jarchi v4.3.1 - Smart installer (fresh install + update in one)
...
REPO_ISSUES="https://github.com/ehsanabdoli/jarchi/issues"
...
echo "=============================================="
echo "  Jarchi v4.3.1 - mode: $([ "$MODE" = "update" ] && echo 'UPDATE existing install' || echo 'FRESH INSTALL')"
echo "=============================================="
...
SED_NOCROSS='s/^playlist_branch = crossfade(duration=4., musicq)$/playlist_branch = musicq/'

if check_engine "$ENGINE_FILE"; then
  echo "   Engine OK (crossfade + ducking)"
elif grep -q "Address already in use" /tmp/liq-check.log; then
  echo "   WARNING: port 1234 busy; assuming script is fine."
else
  echo "   Full variant failed; trying without crossfade..."
  sed "$SED_NOCROSS" "$ENGINE_FILE" > /tmp/radio-nocross.liq
  if check_engine /tmp/radio-nocross.liq; then
    echo "   Engine OK (crossfade disabled)"
    ENGINE_FILE="/tmp/radio-nocross.liq"
  else
...
# هوم کاربر سرویس ممکن است هنوز به مسیر قدیمی اشاره کند
usermod -d "$INSTALL_DIR" "$SVC_USER" 2>/dev/null || true
cd "$INSTALL_DIR/webpanel"
[[ -f .env ]] || cp .env.example .env

# استفاده مجدد از node_modules نصب قبلی (برای سرورهای بدون اینترنت پایدار)
...
if ! sudo -u "$SVC_USER" HOME="$INSTALL_DIR" npm_config_cache="$INSTALL_DIR/.npm-cache" \
     npm install --omit=dev 2>&1 | tail -3; then

...
// اطلاعات عمومی برای فوتر (بدون نیاز به ورود)
// آدرس‌های دونیت ثابت پروژه (مثل روال معمول پروژه‌های متن‌باز، داخل کد)
...
app.get("/meta", (_req, res) => {
  res.json({ app: "Jarchi", version: "4.3.1", author: "Ehsan Abdoli", wallets: DONATE_WALLETS });
});
...
// -----------------------------------------------------------------
// از نسخه ۳.۳ وضعیت (منبع/صدا/آدرس رادیو) در فایل‌های state ذخیره
// می‌شود و موتور صدا خودش آن‌ها را می‌خواند؛ بنابراین حلقه همگام‌سازی
// telnet حذف شد. فقط هنگام بوت پنل، state از دیتابیس بازنویسی
// می‌شود تا فایل‌ها همیشه با دیتابیس هماهنگ باشند.
// -----------------------------------------------------------------
...
// -----------------------------------------------------------------
// ناظر داکینگ: با هر خط start در pages.log، ضریب داکینگ روی مقدار
// تنظیمات و با هر end روی 1.0 برمی‌گردد (+ تایمر ایمنی اگر end گم شد)
// -----------------------------------------------------------------
...
setInterval(() => {
  try {
    ...
    if (st.size < pagesPos) pagesPos = 0; // فایل چرخیده/خالی شده

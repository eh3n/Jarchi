...
// موبایل گاهی کل متن صفحه را paste می‌کند؛ فقط اولین لینک http را نگه دار
function cleanUrl(u) {
...
router.post("/source", async (req, res) => {
  ...
  if (!["radio", "playlist", "spotify", "off"].includes(source))
...
// جلو/عقب بردن آهنگ جاری: position = ثانیه هدف از ابتدای آهنگ
router.post("/transport/seek", async (req, res) => {
  ...
  try {
    ...
    // اگر موتور واقعاً جابه‌جا نکرد، نوار را هم جلو نبر (رفع «تایم می‌رود ولی آهنگ نه»)
...
// بررسی سلامت و کیفیت لینک استریم
...
// میزان کم شدن موزیک هنگام پیج (0 = قطع کامل ... 0.9)
...
router.get("/settings/sip-status", async (_req, res) => {
  try {
    ...
    // اگر سوکت کنترل در دسترس نبود، از مسیر sudo محدود امتحان کن
    try {
...
// بکاپ یک‌کلیکی: دیتابیس + تنظیمات + state (بدون فایل‌های موزیک)
...
router.post("/source-schedules", (req, res) => {
  ...
  if (!/^\d{2}:\d{2}$/.test(time || "") || !["radio", "playlist", "spotify", "off"].includes(source))
...
// ---------------- جستجوی ایستگاه از radio-browser ----------------
...
// عیب‌یابی: وضعیت داخلی موتور صدا + ۵۰ خط آخر لاگ آن
router.get("/system/diagnose", async (_req, res) => {
  ...
  // وضعیت واقعی از فایل‌های state (منبع حقیقت جدید)

// ============================================================
// Jarchi Player v4.1 — مغز ترتیب پخش پلی‌لیست
// موتور صدا فقط یک صف (musicq) دارد؛ این ماژول تصمیم می‌گیرد چه
// آهنگی بعدی است: ترتیبی از روی دیتابیس یا درهم. نتیجه:
//   - بعد از انتخاب دستی آهنگ ۱۰، بعدی واقعاً ۱۱ است (رفع پرش ترتیب)
//   - prev/next/انتخاب همیشه با لیست پنل هماهنگ است
// ============================================================
const fs = require("fs");
...
let lastStarted = null;     // فایل در حال پخش (basename)
let prefetchedAfter = null; // بعدیِ کدام فایل از قبل صف شده
let busyUntil = 0;          // بعد از هر اکشن، چند ثانیه tick کاری نکند
...
// فقط آیتم‌های «در انتظار» را برمی‌گرداند (نه آهنگ در حال پخش)
async function pendingRids() {
  try {
    ...
    try {
      ...
      return (out.match(/\d+/g) || []).slice(1); // اولی معمولاً در حال پخش است
    } catch (_) { return []; }
  }
}
...
async function tick() {
  try {
    ...
    if (valid && (dur ? age < dur + 15 : age < 900)) {
      // آهنگی در حال پخش است → دقیقاً «یک» آیتم بعدی در صف باشد
      ...
      if (prefetchedAfter !== lastStarted) {
        ...
        // اگر از قبل چیزی صف بود (مثلاً بعد از ری‌استارت پنل)، همان را
        // به رسمیت بشناس تا آیتم تکراری اضافه نشود
        prefetchedAfter = lastStarted;
      }
    } else {
      // سکوت/کهنه → فقط اگر موتور خودش چیزی در صف ندارد، راه بینداز
      const pending = await pendingRids();
      if (pending.length === 0) {
